...
contract StorageContract {
	...
	function getEscrow.DC_wallet(bytes32 import_id, address DH_wallet) public view returns(address){
		return escrow[import_id][DH_wallet].DC_wallet;
	}
	function getEscrow.token_amount(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].token_amount;
	}
	function getEscrow.tokens_sent(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].tokens_sent;
	}
	function getEscrow.stake_amount(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].stake_amount;
	}
	function getEscrow.last_confirmation_time(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].last_confirmation_time;
	}
	function getEscrow.end_time(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].end_time;
	}
	function getEscrow.total_time_in_seconds(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].total_time_in_seconds;
	}
	function getEscrow.litigation_interval_in_minutes(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].litigation_interval_in_minutes;
	}
	function getEscrow.litigation_root_hash(bytes32 import_id, address DH_wallet) public view returns(bytes32){
		return escrow[import_id][DH_wallet].litigation_root_hash;
	}
	function getEscrow.distribution_root_hash(bytes32 import_id, address DH_wallet) public view returns(bytes32){
		return escrow[import_id][DH_wallet].distribution_root_hash;
	}
	function getEscrow.checksum(bytes32 import_id, address DH_wallet) public view returns(uint){
		return escrow[import_id][DH_wallet].checksum;
	}
	function getEscrow.escrow_status(bytes32 import_id, address DH_wallet) public view returns(EscrowStatus){
	...
	function setEscrow.DC_wallet(bytes32 import_id, address DH_wallet, uint DC_wallet) public onlyContracts{
		if(escrow[import_id][DH_wallet].DC_wallet != DC_wallet)
		escrow[import_id][DH_wallet].DC_wallet = DC_wallet;
	}
	function setEscrow.token_amount(bytes32 import_id, address DH_wallet, uint token_amount) public onlyContracts{
		if(escrow[import_id][DH_wallet].token_amount != token_amount)
		escrow[import_id][DH_wallet].token_amount = token_amount;
	}
	function setEscrow.tokens_sent(bytes32 import_id, address DH_wallet, uint tokens_sent) public onlyContracts{
		if(escrow[import_id][DH_wallet].tokens_sent != tokens_sent)
		escrow[import_id][DH_wallet].tokens_sent = tokens_sent;
	}
	function setEscrow.stake_amount(bytes32 import_id, address DH_wallet, uint stake_amount) public onlyContracts{
		if(escrow[import_id][DH_wallet].stake_amount != stake_amount)
		escrow[import_id][DH_wallet].stake_amount = stake_amount;
	}
	function setEscrow.last_confirmation_time(bytes32 import_id, address DH_wallet, uint last_confirmation_time) public onlyContracts{
		if(escrow[import_id][DH_wallet].last_confirmation_time != last_confirmation_time)
		escrow[import_id][DH_wallet].last_confirmation_time = last_confirmation_time;
	}
	function setEscrow.end_time(bytes32 import_id, address DH_wallet, uint end_time) public onlyContracts{
		if(escrow[import_id][DH_wallet].end_time != end_time)
		escrow[import_id][DH_wallet].end_time = end_time;
	}
	...
	function setEscrow.litigation_root_hash(bytes32 import_id, address DH_wallet, bytes32 litigation_root_hash) public onlyContracts{
		if(escrow[import_id][DH_wallet].litigation_root_hash != litigation_root_hash)
		escrow[import_id][DH_wallet].litigation_root_hash = litigation_root_hash;
	}
	function setEscrow.distribution_root_hash(bytes32 import_id, address DH_wallet, bytes32 distribution_root_hash) public onlyContracts{
		if(escrow[import_id][DH_wallet].distribution_root_hash != distribution_root_hash)
		escrow[import_id][DH_wallet].distribution_root_hash = distribution_root_hash;
	}
	function setEscrow.checksum(bytes32 import_id, address DH_walle, uint256 checksum) public onlyContracts{
		if(escrow[import_id][DH_wallet].checksum != checksum)
		escrow[import_id][DH_wallet].checksum = checksum;
	}
	function setEscrow.escrow_status(bytes32 import_id, address DH_wallet, EscrowStatus escrow_status) public onlyContracts{
