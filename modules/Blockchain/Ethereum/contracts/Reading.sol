 ...
 */
 ...
 contract Reading is Ownable{
 	...
 	public {
 		...
 		(s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,encrypted_block,s_time_of_sending,s_purchase_status) = Storage.getPurchase(DH_wallet,msg.sender,import_id);
 		
 		require(s_purchase_status == PurchaseStatus.inactive
 			|| 	s_purchase_status == PurchaseStatus.completed);
 		...
 		Storage.setPurchase(DH_wallet,msg.sender,import_id,token_amount,stake_factor,s_dispute_interval_in_minutes,s_commitment,encrypted_block,s_time_of_sending,PurchaseStatus.initiated);
 	...
 	public {
 		...
 		(s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,encrypted_block,s_time_of_sending,s_purchase_status) = Storage.getPurchase(msg.sender,DV_wallet,import_id);

 		require(s_purchase_status == PurchaseStatus.initiated);
 		...
 		// s_purchase_status = PurchaseStatus.commited;

 		Storage.setPurchase(msg.sender,DV_wallet,import_id,s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,commitment,s_encrypted_block,s_time_of_sending,PurchaseStatus.initiated);
 	...
 	public {
 		...
 		// PurchaseDefinition storage this_purchase = purchase[DH_wallet][msg.sender][import_id];
 		(s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,encrypted_block,s_time_of_sending,s_purchase_status) = Storage.getPurchase(DH_wallet,msg.sender,import_id);
 		require(s_purchase_status == PurchaseStatus.commited);


 		// this_purchase.purchase_status = PurchaseStatus.confirmed;
 		Storage.setPurchase(DH_wallet,msg.sender,import_id,s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,s_encrypted_block,s_time_of_sending,PurchaseStatus.confirmed);
 	...
 	public {
 		...
 		(s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,encrypted_block,s_time_of_sending,s_purchase_status) = Storage.getPurchase(DH_wallet,DV_wallet,import_id);

 		require(s_purchase_status == PurchaseStatus.initiated
 			||  s_purchase_status == PurchaseStatus.commited
 			||  s_purchase_status == PurchaseStatus.confirmed);

 		bidding.increaseBalance(DV_wallet, s_token_amount.add(s_token_amount.mul(s_stake_factor)));
 		if(s_purchase_status != PurchaseStatus.initiated){
 		...
 		Storage.setPurchase(DH_wallet,DV_wallet,import_id,s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,s_encrypted_block,s_time_of_sending,PurchaseStatus.cancelled);
 	...
 	public {
 		...
 		(s_DC_wallet,s_distribution_root_hash,s_checksum) = Storage.getPurchasedData(import_id,DV_wallet);


 		// PurchasedDataDefinition storage previous_purchase = purchased_data[import_id][msg.sender];
 		(ps_DC_wallet,ps_distribution_root_hash,ps_checksum) = Storage.getPurchasedData(import_id,msg.sender);

 		require(s_purchase_status == PurchaseStatus.confirmed);
		...
		Storage.setPurchasedData(import_id,msg.sender,ps_DC_wallet,ps_distribution_root_hash,ps_checksum);

 		Storage.setPurchase(DH_wallet,DV_wallet,import_id,s_token_amount,s_stake_factor,s_dispute_interval_in_minutes,s_commitment,encrypted_block,block.timestamp,PurchaseStatus.sent);
